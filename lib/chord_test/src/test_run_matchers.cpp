...
chord_test::matchers::SpawnMachineMatcher::SpawnMachineMatcher(tempo_utils::StatusCode statusCode)
    : m_statusCode(statusCode)
{
}

bool
chord_test::matchers::SpawnMachineMatcher::MatchAndExplain(
    const chord_test::SpawnMachine &spawnMachine,
    std::ostream *os) const
{
    auto exit = spawnMachine.getMachineExit();
    return exit.statusCode == m_statusCode;
}

void
chord_test::matchers::SpawnMachineMatcher::DescribeTo(std::ostream *os) const
{
    *os << "machine exit status code matches " << tempo_utils::status_code_to_string(m_statusCode);
}

void
chord_test::matchers::SpawnMachineMatcher::DescribeNegationTo(std::ostream *os) const
{
    *os << "machine exit status code does not match " << tempo_utils::status_code_to_string(m_statusCode);
}

::testing::Matcher<chord_test::SpawnMachine>
chord_test::matchers::SpawnMachine(tempo_utils::StatusCode statusCode)
{
    return SpawnMachineMatcher(statusCode);
}
